unit BDocumentPartner;

interface
uses Classes, SysUtils, BAddress, BStringArray, BPartnerTaxType, JsonUtils;
type
tBDocumentPartner = class
  private
    p_id: integer;
    p_name: string;
    p_address: tBAddress;
    p_emails: tBStringArray;//string[]',
    p_taxcode: string;
    p_iban: string;
    p_swift: string;
  ...
  public
    ...
    function toString(deep : integer; after: string):string;
...
implementation
constructor tBDocumentPartner.Create;
begin
    p_address:= tBAddress.Create;
end;

destructor tBDocumentPartner.Destroy;
begin
    p_address.Destroy;
    setlength(p_emails,0);
end;

procedure tBDocumentPartner.loadFromJson(data: string);
begin
...
function tBDocumentPartner.getEmails: tBStringArray;
begin
  result:= p_emails;
end;

procedure tBDocumentPartner.setEmails(data: tBStringArray);
begin
  p_emails:= data;
end;

procedure tBDocumentPartner.addEmail(data: string);
begin
  setlength(p_emails, length(p_emails)+1);
  p_emails[length(p_emails)-1]:=data;
end;

function tBDocumentPartner.toJsonStr:string;
var
  s0  : string;
  i0  : integer;
begin
...
function tBDocumentPartner.toString(deep : integer; after: string):string;
var
  i0  : integer;
  deepStr : string;
  lines   : tStringList;
